...
>
  ...
  <BreadcrumbItem href="/login">Register</BreadcrumbItem>
</Breadcrumb>
...
<div class="grid md:grid-cols-3 grid-cols-1 md:gap-3 gap-0 mt-3">
  ...
  <Card size="none" class="mx-3">
    <Heading tag="h2" class="text-center mb-2">Register</Heading>
    <form on:submit={() => (loading = true)} method="POST">
      ...
      <Button class="w-full mb-3" href="/login">
        Login
        <Fa icon={faChevronRight} class="ml-3" />
      </Button>
